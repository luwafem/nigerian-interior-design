  ...
  return (
    <div className="flex min-h-screen flex-col bg-[#FAF9F6] text-[#1A1A1A]">
        ...
        <div className="container mx-auto px-6 flex items-center justify-between">
          ...
          >
            <div className="w-6 h-5 relative flex flex-col justify-between">
              <span className={`h-[1px] w-full bg-current transition-all duration-500 ease-in-out transform ${
        ...
        {isMenuOpen && (
          ...
          >
            <nav className="flex flex-col space-y-6">
              {siteData.navigation.map((item, i) => (
                ...
                >
                  <Link
                    to={item.href}
                    className="font-serif text-5xl italic tracking-tighter hover:text-gray-400 transition-colors"
                  >
                    {item.name}
                  </Link>
                </motion.div>
            ...
            <div className="mt-auto pb-12">
              ...
              <a href={`mailto:${siteData.company.email}`} className="text-lg font-light underline underline-offset-4">
      ...
      <footer className="bg-white border-t border-black/5 py-24 px-6">
        {/* ... Footer content remains same as your previous high-end style ... */}
      </footer>
    </div>
  );
};
